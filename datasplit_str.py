...
read_df.createOrReplaceTempView("COVIDCASES")
...
df_str = spark.sql("select state as name_of_the_state, registerdate as date, count(state) as no_of_occurances from covidcases group by state,date")
...
df_write_int_table.write.format("jdbc")\
    .option("url", my_sql_URL)\
    .option("driver", my_sql_driver)\
    .option("dbtable", "COVID_INT_READ_AGG")\
    .option("user", my_sql_user)\
    .option("password", my_sql_pass)\
    .mode('overwrite').save()
...
df_write_str_table.write.format("jdbc")\
    .option("url", my_sql_URL)\
    .option("driver", my_sql_driver)\
    .option("dbtable", "COVID_STR_READ_AGG")\
    .option("user", my_sql_user)\
    .option("password", my_sql_pass)\
    .mode('overwrite').save()